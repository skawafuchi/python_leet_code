...
class Solution:
    def mergeTwoLists(self, list1: Optional[ListNode],
        ...
        while (True):
            if (pointer_one is not None and pointer_two is None) or \
                ((pointer_one is not None and pointer_two is not None) and
                    pointer_one.val <= pointer_two.val):
                if not return_list:
                ...
                continue
            if (pointer_two is not None and pointer_one is None) or \
                ((pointer_one is not None and pointer_two is not None) and
                    pointer_two.val <= pointer_one.val):
                if not return_list:
